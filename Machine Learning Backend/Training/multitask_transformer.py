...
#model
class vit_b_16_multiTaskNet(nn.Module):
    def __init__(self):
        super(vit_b_16_multiTaskNet, self).__init__()
...
model = vit_b_16_multiTaskNet().to(device)        
...
#lr=0.0001
optimizer = optim.SGD(model.fc_artist.parameters(), lr=0.01,  momentum=0.9, weight_decay =  0.0001)
...
optimizer2 = optim.SGD(model.fc_style.parameters(), lr=0.001,  momentum=0.9, weight_decay =  0.0001)
optimizer3 = optim.SGD(model.fc_genre.parameters(), lr=0.001,  momentum=0.9, weight_decay =  0.0001)
optimizer4 = optim.SGD(model.encoder.parameters(), lr=0.001,  momentum=0.9,  weight_decay =  0.0001)
...
exp_lr_scheduler4 = lr_scheduler.StepLR(optimizer4, step_size=20, gamma=0.1)

T_0 = 10
#exp_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0)
#exp_lr_scheduler2 = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer2, T_0)
#exp_lr_scheduler3 = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer3, T_0)
#exp_lr_scheduler4 = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer4, T_0)


#model.load_state_dict(torch.load("cnn_transfer_learning/modelsavepoints/multitask_trans_freezedlayers_01_0001_0001_45.pth"))
#model.load_state_dict(torch.load("cnn_transfer_learning/modelsavepoints/multitask_trans_freezedlayers_01_0001_0001_60.pth"))
#model.load_state_dict(torch.load("cnn_transfer_learning/modelsavepoints/multitask_trans_freezedlayers_01__lr_60.pth"))
#model.load_state_dict(torch.load("cnn_transfer_learning/modelsavepoints/multitask_trans_freezedlayers_01_nostep_30.pth"))
#model.load_state_dict(torch.load("cnn_transfer_learning/modelsavepoints/vit_b_16_pretrained_cnn_model_45epoch_best_Acc.pth"))
model.load_state_dict(torch.load("cnn_transfer_learning/modelsavepoints/vit_b_16_no_pretrained_cnn_model_45epoch_best_Acc.pth"))

#LR war 0.01 künstler, rest 0.001
...
def train_model(model, criterion, optimizer, optimizer2, optimizer3,optimizer4, exp_lr_scheduler, exp_lr_scheduler2, exp_lr_scheduler3,exp_lr_scheduler4,num_epochs=3 ):
    ...
    save_path = Path("cnn_transfer_learning/modelsavepoints/vit_b_16_no_pretrained_cnn_model_45epoch_best_Acc.pth")
...
plt.xlabel('epoch')
plt.ylabel('accuracy')
plt.legend(['Train_artist','Train_style', 'Train_genre', 'Test_artist', 'Test_style', 'Test_genre'],  bbox_to_anchor=(1.05, 1), loc='upper left')
plt.title('Train vs Test Accuracy')
